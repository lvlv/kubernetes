		...
		if false {
			...
			if yyr130 || yy2arr130 {
				...
				if false {
					...
					r.EncodeBool(bool(x.KernelSupportPropagation))
				}
			} else {
				z.EncSendContainerState(codecSelfer_containerMapKey1234)
				r.EncodeString(codecSelferC_UTF81234, string("kernelSupportPropagation"))
				z.EncSendContainerState(codecSelfer_containerMapValue1234)
				yym470 := z.EncBinary()
				_ = yym470
				if false {
...
func (x *KubeletConfiguration) codecDecodeSelfFromMap(l int, d *codec1978.Decoder) {
	...
	for yyj473 := 0; ; yyj473++ {
		...
		case "kernelSupportPropagation":
			if r.TryDecodeAsNil() {
				x.KernelSupportPropagation = false
			} else {
				x.KernelSupportPropagation = bool(r.DecodeBool())
			}
		default:
